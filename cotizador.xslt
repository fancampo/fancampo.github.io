...
>
	...
	<xsl:key name="type" use="'money'" match="tarifas[not(@tipo_cotizacion='transporte_ganado')]/row/@cuota"/>
	...
	<xsl:key name="hidden" match="*[@xsi:type='mock']/@*" use="generate-id()"/>
	...
	<xsl:key name="readonly" match="@cuota" use="generate-id()"/>
	<xsl:key name="readonly" match="@cuota_total" use="generate-id()"/>
	...
	<xsl:key name="dim" match="/*/estado" use="'estado_origen'"/>
	<xsl:key name="dim" match="/*/municipio" use="'municipio_origen'"/>
	<xsl:key name="dim" match="/*/estado" use="'estado_destino'"/>
	<xsl:key name="dim" match="/*/municipio" use="'municipio_destino'"/>
...
.wizard table > thead.freeze > tr:nth-child(1) > td, .wizard table > thead.freeze > tr:nth-child(1) > th {
	...
	<xsl:template mode="wizard:step-title-legend" match="key('wizard-section','5')">
		<xsl:text>Imprimir cotización</xsl:text>
	</xsl:template>
	...
	<xsl:template mode="wizard:step-panel-content" match="key('wizard-section','1')">
		<xsl:param name="step-number" select="0"/>
		<p>Por favor seleccione el tipo de cotización</p>
	...
	<xsl:template mode="widget" match="municipio|ugr" priority="1">
		...
		<xsl:param name="estado">
			<xsl:apply-templates mode="get-state" select="$context"/>
		</xsl:param>
		<xsl:variable name="options" select="row[@edo=$estado]"/>
		<select class="form-select" xo-scope="{$context/../@xo:id}" xo-slot="{name($context)}" onchange="scope.set(this.value)">
			<option></option>
			<xsl:choose>
